#include <Arduino.h>
#include <arm_library/hardware_pins.h>
#include <arm_library/driver_motor.h>
#include <arm_library/model_encoder.h>
#include <arm_library/model_sensor.h>
...
void setup()
{
    ...
    // Create and initialize input_msg
    input_msg = std_msgs__msg__Float32MultiArray__create();

    // Set up data size
    size_t data_size = 3;  // Based on your example data size

    // Initialize data array
    input_msg->data.data = (float*) allocator.allocate(data_size * sizeof(float), allocator.state);
    input_msg->data.size = data_size;
    input_msg->data.capacity = data_size;
